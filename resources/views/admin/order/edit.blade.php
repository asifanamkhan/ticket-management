            ...
            <form action="{{route('admin.orders.store')}}" method="post">
                ...
                <div class="order-item ">
                    @if(count($order->orderDetails) > 0)
                        @foreach($order->orderDetails as $orderDetailKey => $orderDetail)
                            <div class="flex flex-nowrap gap-2 mt-4">
                                <div class="w-full">
                                    <select class="package-type form-control @error('types') border-theme-24 @enderror"
                                            name="types[]">
                                        <option value="">Select type</option>
                                        @foreach($packageTypes as $typeKey => $type)
                                            <option
                                                value="{{$type->id}}"
                                                @if($type->id == $orderDetail->type_id) selected @endif
                                            >
                                                {{ $type->name }}
                                            </option>
                                        @endforeach
                                ...
                                <div class="w-full">
                                    <select class="days form-control @error('days') border-theme-24 @enderror"
                                            name="days[]">
                                        <option value="">Select Day</option>
                                        @foreach($days as $dayKey => $day)
                                            <option
                                                value="{{$day->id}}"
                                                @if($day->name== $orderDetail->day_name) selected @endif
                                            >
                                                {{ $day->name }}
                                            </option>
                                        @endforeach
                                ...
                                <div class="w-full">
                                    <select class="packages form-control @error('packages') border-theme-24 @enderror"
                                            name="packages[]">
                                        <option value="">Select Package</option>
                                        @foreach($packages as $packageKey => $package)
                                            <option
                                                value="{{$package->id}}"
                                                @if($package->id== $orderDetail->package_id) selected @endif
                                            >
                                                {{ $package->name }}
                                            </option>
                                        @endforeach
                        ...
                        @endforeach
                    @endif
                    <div class="w-full activity-container">
